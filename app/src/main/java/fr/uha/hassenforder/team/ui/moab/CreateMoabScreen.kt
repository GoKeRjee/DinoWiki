...
@Composable
fun CreateMoabScreen(
    vm: MoabViewModel = hiltViewModel(),
    back: () -> Unit,
) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    AppTitle(
                        pageTitleId = R.string.title_moab_create,
        ...
        Column(
            ...
            when (uiState.initialState) {
                MoabViewModel.MoabState.Loading -> {
                    LoadingScreen(text = stringResource(R.string.loading))
                }

                MoabViewModel.MoabState.Error -> {
                    ErrorScreen(text = stringResource(R.string.error))
                }

                is MoabViewModel.MoabState.Success -> {
                    SuccessMoabScreen(uiState, vm.uiCallback)
                }
            }
        }
    }
